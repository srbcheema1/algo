...
int query(int s,int e, int l, int r, int k, int i=1){
     ...
     * returns no of element smaller than k in given range
     */
    if(l==s and r==e){
        ...
        if(pos == (int)tree[i].size()) return pos;// all are smaller return last elem
        return pos;
    }
    ...
    return (query(s,mid,l,mid,k,2*i)+query(mid+1,e,mid+1,r,k,2*i+1));
}
// mst ends here
