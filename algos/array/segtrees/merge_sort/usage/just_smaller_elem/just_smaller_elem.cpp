...
int query(int l,int r, int s, int e, int k, int i=1){ // query(1,n,l,r,k);
    // returns element just smaller than k in given range, INT_MIN if none
    if(l==s and r==e){
        ...
        if(pos == (int)mst[i].size()) return *mst[i].rbegin();// all are smaller return last elem
        return mst[i][pos-1];
    }
    ...
    return max(query(l,mid,s,mid,k,i<<1),query(mid+1,r,mid+1,e,k,i<<1|1));
}
