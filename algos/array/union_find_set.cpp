...
int findSet(int x){
    return (x==p[x])?x:p[x]=findSet(p[x]);// it will shorten the length
}
void refine() {// to set every number to final set
    for(int i=0;i<N;i++) findSet(i);
}
void mergeSet(int x,int y){
    if(findSet(x)!=findSet(y)) x=findSet(x),y=findSet(y), p[max(x,y)]=min(x,y);
}
