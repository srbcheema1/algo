...
void floodfill(int x,int y,int w) {// q contains processed points
    int vis[N][N]; memset(vis,0,sizeof vis);

    queue<point> q;
    q.push({x,y,w});
    vis[x][y] = 1;
    arr[x][y] = max(arr[x][y],w);

    while(!q.empty()) {
        auto t = q.front(); q.pop();
        int x = t.x,y=t.y;
        for(int i=0;i<8;i++) {
            if(not ok(xi,yi)) continue;
...
void flood_fill(int x,int y,int w){// q contains unprocessed points
    int vis[N][N]; memset(vis,0,sizeof vis);

    queue<point> q; q.push({x,y,w});
    while(!q.empty()){
        auto t = q.front(); q.pop();
        x = t.x; y = t.y; w = t.w;
...
void solve()
{
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++) ans = max(arr[i][j],ans);
    }
    cout << ans << endl;
}
