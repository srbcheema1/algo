...
int get_diagonals(int i,int j, int n, int m) {
    int diagonals = 0;//compute it
    diagonals += min(n-i,m-j);
    diagonals += min(i-1,j-1);
    diagonals += min(n-i,j-1);
    diagonals += min(i-1,m-j);
    return diagonals;
}
...
void solve(){
    ...
    for(int i=1;i<=n;i++) {
        for(int j=1;j<=m;j++) {
            if(i==3 and j == 3) debug = true;
            else debug = false;
            if(i==x and j==y) continue;
            int ans = 0;
            ans += nm;
            int diagonals = get_diagonals(i,j,n,m);
            ans -= diagonals;
            bool inway = false;
            show(i,j,ans);
            if(y==j) {
